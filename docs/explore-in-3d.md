...
- **Take the guided tour**: nineteen stops, from a `.scala` file on the outside to machine code in the code cache. A narrator explains each stop (turn the voice or the sound off from the bottom bar if you prefer silence).
